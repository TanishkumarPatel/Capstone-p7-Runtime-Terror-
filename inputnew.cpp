...
int main()
{
   vector <string> inmate_name;
    ...
    vector <pair<string,string>> sleep_time;
    ...
    vector <string> music_channels_id;
        ...
        while(getline(input,perticularline))
        {
            ...
            string name,earpod,sleeptime1,sleeptime2,asleeptime;
            if(getline(iss,name,',') and getline(iss,earpod,',') and getline(iss,sleeptime1,',') and getline(iss,sleeptime2,',') and getline(iss,asleeptime,',') )
            {
                inmate_name.push_back(name);
                earpod_id.push_back(earpod);
                sleep_time.push_back({sleeptime1,sleeptime2});
